...
export function buildTaskboardAutomationPrompt(request) {
  return [
    ...
    "每次仅处理一个 todo：先用 issue get 读取最新任务内容，并用 comment list 读取全部评论，确认是否包含已完成后被打回的返工要求。",
    ...
    "执行完成并验证后，先用 comment add 记录关键改动、验证结果、执行结果和剩余风险，再使用最新 version 将任务移动到 in_review；不要直接标记为 done。",
  ].join("\n");
}
