...
async function deleteTaskAndAttachments(database, task, attachmentsDirectory) {
  const result = database.deleteTask(task.id, task.version);
...
function parseCodexThreadSync(body) {
  assertPlainObject(body);
  assertAllowedKeys(body, new Set(["projectId", "visibleThreadIds"]));
  const projectId = validateProjectId(body.projectId);
  if (!Array.isArray(body.visibleThreadIds) || body.visibleThreadIds.length > 1000) {
    throw new ApiError(400, "INVALID_FIELD", "'visibleThreadIds' must be an array of at most 1000 items");
  }
  const seen = new Set();
  const visibleThreadIds = [];
  for (const value of body.visibleThreadIds) {
    if (typeof value !== "string") {
      throw new ApiError(400, "INVALID_FIELD", "'visibleThreadIds' must contain strings");
    }
    const threadId = normalizeCodexThreadId(value);
    if (!threadId || threadId.startsWith("client-new-thread:") || seen.has(threadId)) continue;
    seen.add(threadId);
    visibleThreadIds.push(threadId);
  }
  return { projectId, visibleThreadIds };
}
...
function parseAiThreadCreate(body) {
  ...
  return {
    ...
    codexThreadId: body.codexThreadId === undefined
      ? undefined
      : stringField(body.codexThreadId, "codexThreadId", { required: true, maxLength: 256 }),
...
async function readCodexProjectMetadata(codexStatePath) {
  try {
    const state = JSON.parse(await readFile(codexStatePath, "utf8"));
    const projects = state["local-projects"];
    if (!projects || typeof projects !== "object" || Array.isArray(projects)) return {};
    return Object.fromEntries(Object.entries(projects).flatMap(([projectId, project]) => {
      if (!project || typeof project !== "object" || Array.isArray(project)) return [];
      const name = typeof project.name === "string" && project.name.trim()
        ? project.name.trim()
        : null;
      const workspacePath = codexProjectRoot(state, projectId);
      if (!name && !workspacePath) return [];
      return [[projectId, { name, workspacePath }]];
    }));
  } catch {
    return {};
  }
...
async function listProjectsWithCodexMetadata(database, codexStatePath) {
  const projects = database.listProjects();
  const codexProjects = await readCodexProjectMetadata(codexStatePath);
  return projects.map((project) => {
    const codexProject = codexProjects[project.id];
    const name = codexProject?.name ?? project.name;
    const workspacePath = project.workspacePath ?? codexProject?.workspacePath ?? null;
    return name === project.name && workspacePath === project.workspacePath
      ? project
      : { ...project, name, workspacePath };
  });
}

async function readCodexThreadTitle(codexSessionIndexPath, threadId) {
  try {
    const lines = (await readFile(codexSessionIndexPath, "utf8")).split(/\r?\n/);
    let title = null;
...
function addCodexThreadId(target, value) {
  const threadId = normalizeCodexThreadId(value);
  if (threadId && !threadId.startsWith("client-new-thread:")) target.add(threadId);
}

async function readCodexThreadSyncState(resolved, projectId) {
  const sidebarThreadIds = new Set();
  const deletedThreadIds = new Set();
  const knownThreadIds = new Set();
  let hasSidebarProject = false;
  try {
    const state = JSON.parse(await readFile(resolved.codexStatePath, "utf8"));
    const projectOrder = state["sidebar-project-thread-orders"]?.[projectId];
    if (projectOrder && typeof projectOrder === "object" && Array.isArray(projectOrder.threadIds)) {
      hasSidebarProject = true;
      for (const threadId of projectOrder.threadIds) addCodexThreadId(sidebarThreadIds, threadId);
    }
    const deletedPrefix = "codex-writing-block-deleted-thread-v1:";
    for (const key of Object.keys(state)) {
      if (key.startsWith(deletedPrefix) && state[key]) addCodexThreadId(deletedThreadIds, key.slice(deletedPrefix.length));
    }
  } catch {}

  try {
    const lines = (await readFile(resolved.codexSessionIndexPath, "utf8")).split(/\r?\n/);
    for (const line of lines) {
      if (!line.trim()) continue;
      try {
        addCodexThreadId(knownThreadIds, JSON.parse(line)?.id);
      } catch {}
    }
  } catch {}

  try {
    for (const filename of await readdir(resolved.codexArchivedSessionsPath)) {
      const match = /([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})\.jsonl$/i.exec(filename);
      if (match) addCodexThreadId(knownThreadIds, match[1]);
    }
  } catch {}

  return { hasSidebarProject, sidebarThreadIds, knownThreadIds, deletedThreadIds };
}

async function syncTasksForCodexThreads(database, resolved, input, events) {
  const state = await readCodexThreadSyncState(resolved, input.projectId);
  const visibleThreadIds = state.hasSidebarProject
    ? state.sidebarThreadIds
    : new Set(input.visibleThreadIds);
  if (!state.hasSidebarProject && visibleThreadIds.size === 0) return;

  const tasks = database.listTasks({ projectId: input.projectId, archived: "false" });
  for (const task of tasks) {
    const threadId = normalizeCodexThreadId(task.threadId);
    if (!threadId || visibleThreadIds.has(threadId)) continue;
    if (state.deletedThreadIds.has(threadId) || !state.knownThreadIds.has(threadId)) {
      const deleted = await deleteTaskAndAttachments(database, task, resolved.attachmentsDirectory);
      events.emit("task.deleted", { task: deleted });
    } else {
      const archived = database.archiveTask(task.id, task.version, task.threadId);
...
export function resolveServerOptions(options = {}) {
  ...
  return {
    ...
    codexSessionIndexPath: options.codexSessionIndexPath
      ?? path.join(codexHome, "session_index.jsonl"),
    codexArchivedSessionsPath: options.codexArchivedSessionsPath
  ...
  const server = createServer(async (request, response) => {
    ...
    try {
      ...
      if (projectMappingRoute) {
        ...
        const project = database.mapProjectWorkspace(projectId, workspacePath);
        return sendJson(response, 200, { projectId, workspacePath, project });
      }
      ...
      if (pathname === "/api/local/codex/threads/sync") {
        if (request.method !== "POST") return methodNotAllowed(response, ["POST"]);
        assertNoQuery(url.searchParams, "POST /api/local/codex/threads/sync");
        await syncTasksForCodexThreads(database, resolved, parseCodexThreadSync(await readJson(request)), events);
        return sendEmpty(response, 204);
      }

      const codexThreadRoute = pathname.match(/^\/api\/local\/codex\/threads\/([^/]+)$/);
      if (codexThreadRoute) {
        if (request.method !== "GET") return methodNotAllowed(response, ["GET"]);
        assertNoQuery(url.searchParams, "GET /api/local/codex/threads/:id");
        const threadId = parseThreadId(decodeRouteSegment(codexThreadRoute[1], "Thread id"));
        return sendJson(response, 200, {
          thread: {
            id: threadId,
            title: await readCodexThreadTitle(resolved.codexSessionIndexPath, threadId),
          },
        });
      }
      ...
      if (aiThreadRoute) {
        ...
        if (request.method === "DELETE") {
          ...
          const thread = await aiChat.deleteThread(threadId);
          const linkedTask = thread.origin.issueId ? database.getTask(thread.origin.issueId) : null;
          if (linkedTask) {
            const deleted = await deleteTaskAndAttachments(database, linkedTask, resolved.attachmentsDirectory);
            events.emit("task.deleted", { task: deleted });
          }
          return sendEmpty(response, 204);
        }
      ...
      if (pathname === "/api/projects") {
        if (request.method === "GET") {
          ...
          return sendJson(response, 200, {
            projects: await listProjectsWithCodexMetadata(database, resolved.codexStatePath),
          });
        }
        if (request.method === "POST") {
      ...
      if (projectArchiveRoute) {
        if ([...url.searchParams.keys()].length > 0) {
          throw new ApiError(400, "UNKNOWN_QUERY_PARAMETER", "Project archive route does not accept query parameters");
        }
        let projectId;
        try {
          projectId = decodeURIComponent(projectArchiveRoute[1]);
        } catch {
          throw new ApiError(400, "INVALID_PATH", "Project id contains invalid encoding");
        }
        validateProjectId(projectId);
        if (request.method === "POST") {
          const project = database.archiveProject(projectId);
          events.emit("project.archived", { project });
          return sendJson(response, 200, { project });
        }
        return methodNotAllowed(response, ["POST"]);
      }

      const projectDeleteRoute = pathname.match(/^\/api\/projects\/([^/]+)$/);
      if (projectDeleteRoute) {
        if ([...url.searchParams.keys()].length > 0) {
          throw new ApiError(400, "UNKNOWN_QUERY_PARAMETER", "Project delete route does not accept query parameters");
        }
        let projectId;
        try {
          projectId = decodeURIComponent(projectDeleteRoute[1]);
        } catch {
          throw new ApiError(400, "INVALID_PATH", "Project id contains invalid encoding");
        }
        validateProjectId(projectId);
        if (request.method === "DELETE") {
          const result = database.deleteProject(projectId);
          for (const attachment of result.attachments) {
            try {
          ...
          events.emit("project.deleted", { project: result.project });
          return sendEmpty(response, 204);
        }
      ...
      if (taskRoute) {
        ...
        if (!action && request.method === "DELETE") {
          const { version } = parseArchive(await readJson(request));
          const task = await deleteTaskAndAttachments(database, { id, version }, resolved.attachmentsDirectory);
          events.emit("task.deleted", { task });
          return sendEmpty(response, 204);
        }
        if (action === "move" && request.method === "POST") {
          const move = parseMove(await readJson(request));
          const previousProjectId = move.projectId === undefined ? undefined : database.getTask(id).projectId;
          const task = database.moveTask(id, move.version, move.status, move.sortOrder, move.threadId, move.projectId);
          events.emit("task.moved", { task, previousProjectId });
          return sendJson(response, 200, { task });
        }
