...
import { DEFAULT_PROJECT_ID, DEFAULT_PROJECT_NAME } from "../shared/domain.mjs";
...
export class TaskboardDatabase {
  ...
  #migrate() {
    ...
    `).run(DEFAULT_PROJECT_ID, DEFAULT_PROJECT_NAME, timestamp, timestamp);
    this.database.prepare(`
      UPDATE projects
      SET name = ?, updated_at = ?
      WHERE id = ? AND name IN ('Local', '无项目议题')
    `).run(DEFAULT_PROJECT_NAME, timestamp, DEFAULT_PROJECT_ID);
  }
  ...
  mapProjectWorkspace(id, workspacePath) {
  ...
  archiveProject(id) {
    if (id === DEFAULT_PROJECT_ID) {
      throw new ApiError(409, "DEFAULT_PROJECT_PROTECTED", "The default project cannot be archived or deleted");
    }
    const current = this.getProject(id);
    if (!current) throw new ApiError(404, "PROJECT_NOT_FOUND", `Project '${id}' does not exist`);
  ...
  deleteProject(id) {
    ...
    this.database.exec("BEGIN IMMEDIATE");
    try {
      attachments = this.database.prepare(`
        SELECT attachments.*
        FROM attachments
  ...
  getWorkflowWorkspace(projectId) {
    if (!this.database.prepare("SELECT 1 FROM projects WHERE id = ? AND archived_at IS NULL").get(projectId)) {
  ...
  createTask(input) {
    ...
    try {
      const project = this.database.prepare(`
        SELECT id, next_task_number FROM projects WHERE id = ? AND archived_at IS NULL
      `).get(input.projectId);
      if (!project) {
      ...
      this.database.prepare(`
        UPDATE projects SET next_task_number = next_task_number + 1, updated_at = ? WHERE id = ? AND archived_at IS NULL
  ...
  moveTask(id, version, status, sortOrder, threadId, projectId) {
    const current = this.#requireTask(id);
    this.#requireVersion(current, version);
    if (current.archivedAt !== null) {
      throw new ApiError(409, "TASK_ARCHIVED", "Archived tasks cannot be moved");
    }
    const sourceProjectId = current.projectId;
    const targetProjectId = projectId ?? sourceProjectId;
    if (!this.database.prepare("SELECT 1 FROM projects WHERE id = ? AND archived_at IS NULL").get(targetProjectId)) {
    ...
    if (sortOrder === undefined) {
      ...
      `).get(targetProjectId, status, current.id);
      sortOrder = row.maximum + 1000;
    }
    ...
    try {
      const result = this.database.prepare(`
        UPDATE tasks
        SET project_id = ?, status = ?, sort_order = ?, thread_id = COALESCE(?, thread_id),
            version = version + 1, updated_at = ?
        WHERE id = ? AND version = ?
      `).run(targetProjectId, status, sortOrder, threadId ?? null, timestamp, current.id, version);
      if (result.changes !== 1) {
        this.#throwMissingOrConflict(id, version);
      }
      this.database.prepare("UPDATE projects SET updated_at = ? WHERE id = ?").run(timestamp, sourceProjectId);
      if (targetProjectId !== sourceProjectId) {
        this.database.prepare("UPDATE projects SET updated_at = ? WHERE id = ?").run(timestamp, targetProjectId);
      }
      this.database.exec("COMMIT");
    } catch (error) {
      this.database.exec("ROLLBACK");
  ...
  deleteTask(id, version) {
    ...
    try {
      attachments = this.database.prepare(`
        ...
        WHERE task_id = ?
      `).all(current.id).map(attachmentFromRow);
      const result = this.database.prepare(`
        DELETE FROM tasks
        WHERE id = ? AND version = ?
    ...
    return { task: current, attachments };
  }
