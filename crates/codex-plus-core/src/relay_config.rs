...
pub fn ensure_active_protocol_proxy_config_in_home(
    ...
    let openai_identity_uses_proxy = settings.relay_profiles_enabled
        && (settings.active_relay_session_provider() == RelaySessionProvider::Openai
...
fn align_profile_model_with_active_goal_thread(
    home: &Path,
    profile: &RelayProfile,
) -> RelayProfile {
...
#[doc(hidden)]
pub fn preserve_live_app_settings_for_test(
    home: &Path,
    config_text: &str,
) -> anyhow::Result<String> {
...
fn copy_standard_responses_catalog(
    ...
    for model in models {
        ...
        if !lite_supported && model.get("use_responses_lite").and_then(Value::as_bool) == Some(true)
        {
...
"#;
        ...
        assert_eq!(
            doc["mcp_servers"]["cua_repl"]["command"].as_str(),
            Some("cua")
        );
    }
